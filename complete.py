...
#verifying input
        ...
        def check():
                    ...
                    self.label1 = Label(self, text="Username" , bg = 'gray' , fg = 'black', font=("courier-new", 18, 'bold'))
...
class Add(Tk):
    def __init__(self):
        ...
        self.title('Add Book')
        self.canvas = Canvas(width=1920, height=1080, bg='gray')
        self.canvas.pack()
        a = StringVar()
        b = StringVar()
        c = StringVar()
            ...
            if len(b.get()) == 0 or len(c.get()) == 0:
                ...
                try:
                    self.conn = mysql.connector.connect(host='localhost',
                                         ...
                                         password='heelo.123')
                    self.myCursor = self.conn.cursor()
                    self.myCursor.execute("Insert into book(book_id,name,author,availability) values (%s,%s,%s,%s)",[a.get(),b.get(),c.get(),g])
                    self.conn.commit()
        ...
        Label(self, text='').pack()
        Label(self, text='Book ID:',bg='gray',fg='black', font=('Courier new', 10, 'bold')).place(x=60, y=180)
        Entry(self, textvariable=a, width=30).place(x=170, y=182)
        Label(self, text='Book Name:',bg='gray',fg='black', font=('Courier new', 10, 'bold')).place(x=60, y=230)
        Entry(self, textvariable=b, width=30).place(x=170, y=232)
        Label(self, text='Book Author:',bg='gray',fg='black', font=('Courier new', 10, 'bold')).place(x=60, y=280)
        Entry(self, textvariable=c, width=30).place(x=170, y=282)
        Button(self, text="Submit", command=b_q).place(x=245, y=330)
Add().mainloop()
             ...
             try:
                    ...
                    try:
                     if self.pc:
                        ...
                        self.mycursor.execute("Insert into issue_book(issue_id, book_id,stud_id,issue_date,return_date) values (%s,%s,%s,%s,%s)",
                                              [isd,book, stud, idate,''])
                        self.conn.commit()
...
#label and input box
        ...
        Label(self, text='Issue ID:',bg = 'gray', font=('Courier new', 15), fg='black').place(x=55, y=100)
        Entry(self, textvariable=k, width=40).place(x=160, y=106)
        Label(self, text='Book ID:',bg = 'gray', font=('Courier new', 15), fg='black').place(x=55, y=150)
        Entry(self, textvariable=c, width=40).place(x=160, y=156)
        Label(self, text='Student ID:',bg = 'gray', font=('Courier new', 15), fg='black').place(x=20, y=200)
        Entry(self, textvariable=d, width=40).place(x=160, y=205)
        Button(self, text="ISSUE", width=20, command=isb).place(x=200, y=250)
issue().mainloop()
        ...
        def insert():
            try:
                self.conn = mysql.connector.connect(host='localhost',
                                         ...
                                         password='heelo.123')
                self.myCursor = self.conn.cursor()
                self.myCursor.execute("Insert into admin(id, user,name,password) values (%s, %s,%s,%s)",[i.get(), u.get(), n.get(), p.get()])
                self.conn.commit()
...
#label and input
        ...
        Label(self, text='User Id:', bg='gray', font=('Courier new', 10, 'bold')).place(x=70, y=82)
        Entry(self, textvariable=i, width=30).place(x=200, y=84)
        Label(self, text='Username:', bg='gray', font=('Courier new', 10, 'bold')).place(x=70, y=130)
        Entry(self, textvariable=u, width=30).place(x=200, y=132)
        Label(self, text='Name:', bg='gray', font=('Courier new', 10, 'bold')).place(x=70, y=180)
        Entry(self, textvariable=n, width=30).place(x=200, y=182)
        Label(self, text='Password:', bg='gray', font=('Courier new', 10, 'bold')).place(x=70, y=220)
        Entry(self, textvariable=p, width=30).place(x=200, y=222)
        Button(self, text="Submit", width=15, command=insert).place(x=230, y=270)
reg().mainloop()
